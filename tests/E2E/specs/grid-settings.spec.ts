...
test.describe('Grid settings tab', () => {
  ...
  test('editor sets a per-viewport override through the Grid tab, confirms persistence, then clears it', async ({
    ...
    const overrideToggle = page.locator(
      `input[name="GridSettings[${overrideKey}][override]"]`,
    );
    const overrideWidthSelect = page.locator(
      `select[name="GridSettings[${overrideKey}][width]"]`,
    );
    const overrideVisible = page.locator(
      `input[name="GridSettings[${overrideKey}][visible]"]`,
    );
