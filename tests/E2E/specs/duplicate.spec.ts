...
test.describe('Duplicate element', () => {
  ...
  test('duplicate section to different page and zone via dialog', async ({ page }) => {
    ...
    await test.step('Search and select target page', async () => {
      ...
      await expect(
        dialog.getByTestId('duplicate-to-page-item').filter({ hasText: 'E2E Duplicate Target Page' }),
      ).toBeVisible({ timeout: 5_000 });

      await dialog.getByTestId('duplicate-to-page-item').filter({ hasText: 'E2E Duplicate Target Page' }).click();
    ...
    await test.step('Navigate to target page and verify copy in sidebar zone', async () => {
      ...
      const copy = sidebarZone.getByTestId('section-block').filter({ hasText: 'Source Section copy' });
  ...
  test('duplicate content element to different column on same page via dialog', async ({ page }) => {
    const fixture = await loadAndNavigate(page, 'duplicate-test');
    ...
    await test.step('Single zone auto-skips to container step', async () => {
      ...
      await expect(dialog.getByTestId('duplicate-to-step-container')).toBeVisible({ timeout: 5_000 });
    });
    ...
    await test.step('Verify Col B now contains both Element Beta and Element Alpha copy', async () => {
      ...
      await expect(colB.getByTestId('element-card').filter({ hasText: 'Element Beta' })).toBeVisible();
      await expect(colB.getByTestId('element-card').filter({ hasText: 'Element Alpha copy' })).toBeVisible();
    });
