...
test.describe('Cross-row column drop positions', () => {
  ...
  test('moves columns across rows in both directions', async ({ page }) => {
    ...
    await test.step('Forward, before-first: A1 → Row B before B1', async () => {
      await activateDragByTitle(page, 'Col A1', { axis: 'horizontal', overlayTestId: 'drag-overlay-column' });
      await enterRow(page, rowB, 4);
      const pos = await colPosition(page, rowB, { before: 'Col B1' });
      await dropAndSettle(page, pos.x, pos.y);
      await expect.poll(() => getColumnTitles(rowB)).toEqual(['Col A1', 'Col B1', 'Col B2', 'Col B3']);
    });

    // Row B [A1, B1, B2, B3]  →  Row B [A1, B1, B2]
    // Row A [A2, A3]           →  Row A [A2, *B3*, A3]
    await test.step('Reverse, between: B3 → Row A between A2,A3', async () => {
      await activateDragByTitle(page, 'Col B3', { axis: 'horizontal', overlayTestId: 'drag-overlay-column' });
    ...
    await test.step('Reverse, after-last: B2 → Row A after A3', async () => {
      await activateDragByTitle(page, 'Col B2', { axis: 'horizontal', overlayTestId: 'drag-overlay-column' });
      await enterRow(page, rowA, 4);
      const pos = await colPosition(page, rowA, { after: 'Col A3' });
      await dropAndSettle(page, pos.x, pos.y);
      await expect.poll(() => getColumnTitles(rowA)).toEqual(['Col A2', 'Col B3', 'Col A3', 'Col B2']);
    });
    ...
    await test.step('Forward, after-last: A3 → Row B after B1', async () => {
      await activateDragByTitle(page, 'Col A3', { axis: 'horizontal', overlayTestId: 'drag-overlay-column' });
      await enterRow(page, rowB, 4);
      const pos = await colPosition(page, rowB, { after: 'Col B1' });
      await dropAndSettle(page, pos.x, pos.y);
      await expect.poll(() => getColumnTitles(rowB)).toEqual(['Col A1', 'Col B2', 'Col B1', 'Col A3']);
    });
    ...
    await expect.poll(() => getColumnTitles(rowBReloaded)).toEqual(['Col A1', 'Col B2', 'Col B1', 'Col A3']);
  });
  ...
  test('handles edge cases: source depletion and cancel mid-drag', async ({ page }) => {
    ...
    await test.step('Source depletion: move lone column to target', async () => {
      ...
      await expect.poll(() => getColumnTitles(rowB)).toEqual(['Col B1', 'Col A1', 'Col B2', 'Col B3']);

      // Verify persistence
      await page.reload();
      await expect(page.getByTestId('grid-editor-loading')).toBeHidden({ timeout: 15_000 });
      const rowBReloaded = getRow(page, 'Row B');
      await expect.poll(() => getColumnTitles(rowBReloaded)).toEqual(['Col B1', 'Col A1', 'Col B2', 'Col B3']);
    });
