...
test.describe('History view readonly grid', () => {
  ...
  test('editor publishes, edits, re-publishes, then sees the grid in history', async ({ page }) => {
    ...
    await editGridEditor.getByTestId('add-child-append').filter({ hasText: 'Add Section' }).click()
    ...
    await expect(
      historyGridEditor.getByTestId('section-block').filter({ hasText: 'Original Alpha Section' }),
    ).toBeVisible()
