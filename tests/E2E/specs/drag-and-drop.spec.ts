...
test.describe('Drag and drop', () => {
  ...
  test('rearranges content at every hierarchy level and persists across reload', async ({
    ...
    // Hoisted: captured in the section-reorder step, asserted in the reload step.
    ...
    await test.step('reorder elements within and across columns', async () => {
    ...
    await test.step('reorder columns and rows', async () => {
    ...
    await test.step('move a row across sections and reorder sections', async () => {
      ...
      sectionTitles = await sections.getByTestId('section-title').allTextContents()
    })

    await test.step('persist reordering across reload and publish to the frontend', async () => {
      // --- HARD RELOAD ---
      await page.reload()
      await expect(page.getByTestId('grid-editor-loading')).toBeHidden({ timeout: 15_000 })
