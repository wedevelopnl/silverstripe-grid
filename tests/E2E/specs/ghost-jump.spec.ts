...
test.describe('Ghost jump regression', () => {
  ...
  test('row does not swap until dragged center crosses sibling center', async ({ page }) => {
    ...
    // Hoisted so both steps (the drag starts in the first, the assertions and
    // release run in the second) share the same baseline measurements.
    let fromX = 0
    let fromY = 0
    let initialY = 0

    await test.step('measure baseline positions and activate the drag', async () => {
      ...
      fromX = handleBox!.x + handleBox!.width / 2
      fromY = handleBox!.y + handleBox!.height / 2
    ...
    await test.step('row swaps only after the dragged center crosses the sibling center', async () => {
