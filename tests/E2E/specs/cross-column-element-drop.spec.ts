...
test.describe('Cross-column element drop — both directions', () => {
  ...
  test('moves elements across columns in both directions', async ({ page }) => {
    ...
    await test.step('Forward, before-first: A1 → Col B before B1', async () => {
      ...
      await dropAndSettle(page, pos.x, pos.y, 'Forward, before-first: A1 → Col B before B1')
      await expect(colB.getByTestId('element-card-title')).toHaveText([
        'Element A1',
        'Element B1',
        'Element B2',
    ...
    await test.step('Reverse, between: B3 → Col A between A2,A3', async () => {
      ...
      await dropAndSettle(page, pos.x, pos.y, 'Reverse, between: B3 → Col A between A2,A3')
      await expect(colA.getByTestId('element-card-title')).toHaveText([
        'Element A2',
        'Element B3',
        'Element A3',
    ...
    await test.step('Reverse, after-last: B2 → Col A after A3', async () => {
      ...
      await dropAndSettle(page, pos.x, pos.y, 'Reverse, after-last: B2 → Col A after A3')
      await expect(colA.getByTestId('element-card-title')).toHaveText([
        'Element A2',
        'Element B3',
        'Element A3',
    ...
    await test.step('Forward, between: B2 → Col B between A1,B1', async () => {
      ...
      await dropAndSettle(page, pos.x, pos.y, 'Forward, between: B2 → Col B between A1,B1')
      await expect(colB.getByTestId('element-card-title')).toHaveText([
        'Element A1',
        'Element B2',
        'Element B1',
    ...
    await test.step('Forward, after-last: A3 → Col B after B1', async () => {
      ...
      await dropAndSettle(page, pos.x, pos.y, 'Forward, after-last: A3 → Col B after B1')
      await expect(colB.getByTestId('element-card-title')).toHaveText([
        'Element A1',
        'Element B2',
        'Element B1',
    ...
    await test.step('Intra-container: B3 before A2 in Col A', async () => {
      ...
      await dropAndSettle(page, pos.x, pos.y, 'Intra-container: B3 before A2 in Col A')
    ...
    await expect(colBReloaded.getByTestId('element-card-title')).toHaveText([
      'Element A1',
      'Element B2',
      'Element B1',
      'Element A3',
...
test.describe('Cross-column element drop — source depletion and cancel', () => {
  ...
  test('handles edge cases: source depletion and cancel mid-drag', async ({ page }) => {
    ...
    await test.step('Source depletion: move lone element to target', async () => {
      ...
      await dropAndSettle(page, pos.x, pos.y, 'Source depletion: move lone element to target')
    ...
    await test.step('Source depletion reverse: lone element B1 → Col A after A3', async () => {
      ...
      await dropAndSettle(
        ...
        pos.x,
        pos.y,
        'Source depletion reverse: lone element B1 → Col A after A3',
      )
