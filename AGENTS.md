...
- **CLAUDE.md and AGENTS.md are regenerated from `.apm/instructions/` on every `apm compile`** — direct edits to the generated files get overwritten silently. Edit the APM sources instead. `GEMINI.md` is generated too, but only as a stub that includes `@./AGENTS.md`, so its build ID stays stale across content changes — that is not drift.
...
- **Vite lib mode inlines every resolvable `url()` as base64 and ignores `assetsInlineLimit`**: `fonts.css` therefore points at `../fonts/…`, a path relative to the BUILT stylesheet that deliberately does not resolve from `client/src/styles/`. Vite logs "didn't resolve at build time" per face and emits the URL verbatim; the `ssgrid-copy-fonts` plugin in `vite.config.mts` copies `client/fonts/` → `client/dist/fonts/`. Those six build warnings are the mechanism working. "Fixing" the path re-inlines ~54 KB of base64 into the render-blocking stylesheet and makes `unicode-range` subsetting a no-op.
...
- **The container harness installs from a COMMITTED lock — never hand-edit it.** `.docker/app/composer.lock` and `composer.fluent.lock` are COPYed into the image; the boot install replays them instead of re-resolving against Packagist (which cost 46 red CI jobs in one month, all reported as bare `exited (100)`). Regenerate with `task relock`, which resolves inside the image; a weekly workflow raises the same change as a PR so the harness keeps following SS6.x. Both files must exist or the image build fails at `COPY`.
- **Composer does NOT re-check a path package's requires when installing from a lock.** Add a `require` to the module's own `composer.json` without running `task relock` and `composer install` prints nothing, exits 0, and omits the package — surfacing later as an unrelated class-not-found. `task verify-lock` (a CI gate in the static-analysis job) is what catches it. Verified against composer 2.10.
- **`config.platform.php` in both harness composer.json files is pinned to `8.3.0`.** One lock serves the 8.3/8.4/8.5 matrix legs, so it must hold a set installable on the lowest. Raising the pin without raising the module's own PHP floor makes the 8.3 leg refuse to install.
