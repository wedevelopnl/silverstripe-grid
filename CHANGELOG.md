...
- **BREAKING (upgrade step required): `Zone` moved from the root-element subclasses to `GridElement`** — `Zone` was declared on `Section` and again on `SharedBlockReference`, each with its own index, and is meaningful only when the element's parent is a page. Because it lived on subclass tables, `GridElement::get()` could not filter it, so every read of a page's root elements fanned out to one query per root class and merged the slices in PHP (`OrmGridElementRepository::findByParents()`), and every root-level Sort assignment took a `max('Sort')` per class (`GridElement::ensureSortSet()`). `ElementPlacementService` had to read the value through an untyped `getField('Zone')` because the base class had no property. It is now one column on the base table with a `(ParentClass, ParentID, Zone, Sort)` index alongside the existing `(ParentClass, ParentID, Sort)`, so a zone's whole root sequence comes back from a single indexed read and `$element->Zone` is typed. `GridElement::getZoneValue()` and `ElementPlacementService::zoneOf()` are gone (both existed only to work around the subclass column); `Section::getCMSFields()` is gone too, since the base class now hides the scaffolded field. Behaviour is unchanged.

  **Upgrading:** run `dev/build`, then `sake dev/tasks/backfill-grid-zone` (add `--dry-run` to preview). `dev/build` creates the new base column EMPTY and leaves the old values behind, so until the task runs every page's zone assignment reads as `''` and the grid disappears from its zones. The task is idempotent, never overwrites a zone already set on the base, and handles both shapes `dev/build` leaves behind: `Section` was left with no own `$db` fields so its tables were renamed to `_obsolete_…`, while `SharedBlockReference` still declares `BlockID` so its tables survived with `Zone` orphaned on them. All three stages (draft, `_Live`, `_Versions`) are copied.
