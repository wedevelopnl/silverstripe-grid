...
const KEY_PREFIX = "WeDevelopGrid.";
const I18N_MODULE_SUFFIX = "i18n";
...
function isI18nImport(modulePath) {
  return (
...
function collect() {
  ...
  let existingEn = {};
  try {
    existingEn = JSON.parse(readFileSync(LANG_SRC_EN, "utf8") || "{}");
  } catch {
...
function emit(found) {
  ...
  writeFileSync(LANG_SRC_EN, `${JSON.stringify(sortedEn, null, 2)}\n`);

  const nlRaw = readFileSync(LANG_SRC_NL, "utf8") || "{}";
  const nl = JSON.parse(nlRaw);
  const sortedNl = Object.fromEntries(Object.keys(nl).sort().map((k) => [k, nl[k]]));
  writeFileSync(LANG_SRC_NL, `${JSON.stringify(sortedNl, null, 2)}\n`);
  ...
  writeFileSync(LANG_RUNTIME_EN, enBundle);
  writeFileSync(LANG_RUNTIME_NL, nlBundle);
}
